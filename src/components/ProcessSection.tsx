        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          <div className="flex items-center gap-2 bg-white p-2 rounded-2xl border border-slate-200/90 shadow-sm self-start md:self-end">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-purple-50 hover:bg-purple-100 text-[#7C5CFC] text-xs font-bold transition-all cursor-pointer"
              title={isAutoPlaying ? "Pause Automated Walkthrough" : "Play Automated Walkthrough"}
            >
              {isAutoPlaying ? (
                <>
                  <Pause className="w-3.5 h-3.5 text-[#7C5CFC]" />
          ...
          <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden flex gap-1 p-0.5">
            {processSteps.map((step, idx) => {
              ...
              return (
                ...
                >
                  {isPast && <div className="h-full w-full bg-[#7C5CFC]" />}
                  {isCurrent && (
                    <motion.div
                      className="h-full bg-gradient-to-r from-[#7C5CFC] to-indigo-600 rounded-full"
              ...
              return (
                <motion.div
                  ...
                  className={`relative rounded-3xl p-5 sm:p-6 transition-all duration-300 flex flex-col items-start cursor-pointer select-none ${
                    isActive
                      ? "bg-white shadow-xl shadow-purple-500/10 border-2 border-[#7C5CFC] ring-4 ring-purple-100"
                      : "bg-white/80 hover:bg-white border border-slate-200/80 shadow-sm hover:shadow-md"
                  }`}
                >
                  {isActive && (
                    <div className="absolute -top-3 right-4">
                      <span className="px-2.5 py-0.5 rounded-full bg-[#7C5CFC] text-white text-[9px] font-black uppercase tracking-wider shadow-xs flex items-center gap-1">
                  ...
                  <div className="flex items-center justify-between w-full mb-5">
                    ...
                    <span className={`text-xl font-black tracking-tight ${isActive ? "text-[#7C5CFC]" : "text-slate-300"}`}>
                      {step.step}
                    </span>
                  </div>

                  <h3 className={`text-lg font-black mb-1 transition-colors ${isActive ? "text-[#7C5CFC]" : "text-slate-900"}`}>
                    {step.title}
                  </h3>
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">{step.tagline}</p>
                  ...
                  <div className="mt-auto pt-3 border-t border-slate-100 w-full flex items-center justify-between text-[11px] font-bold">
                    ...
                    <span className={`flex items-center gap-0.5 transition-colors ${isActive ? "text-[#7C5CFC]" : "text-slate-400"}`}>
                      <span>Details</span>
                      <ChevronRight className={`w-3.5 h-3.5 ${isActive ? "translate-x-0.5 text-[#7C5CFC]" : ""}`} />
                    </span>
                  </div>
                </motion.div>
              );
            ...
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              <div className="lg:col-span-7 space-y-6">
                <div className="flex flex-wrap items-center gap-2.5">
                  <span className={`px-3 py-1 rounded-xl text-xs font-black uppercase tracking-wider flex items-center gap-1.5 ${activeStep.accentBg}`}>
                ...
                <div className="flex flex-wrap items-center gap-2 pt-2">
                  ...
                  {activeStep.toolsUsed.map((tool) => (
                    <span
                      key={tool}
                      className="px-2.5 py-1 rounded-lg bg-purple-50 text-[#7C5CFC] text-xs font-bold border border-purple-200/60"
                    >
                      {tool}
                    </span>
                  ))}
              ...
              <div className="lg:col-span-5 flex flex-col justify-center space-y-4">
                ...
                <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 border border-slate-200 text-xs">
                  <button
                    onClick={handlePrev}
                    className="flex items-center gap-1 font-bold text-slate-600 hover:text-[#7C5CFC] transition-colors cursor-pointer"
                  >
                    <ArrowLeft className="w-3.5 h-3.5" />
                    <span>Previous Phase</span>
                  </button>
                  <span className="font-extrabold text-slate-400">{activeStep.step} / 05</span>
                  <button
                    onClick={handleNext}
                    className="flex items-center gap-1 font-bold text-[#7C5CFC] hover:text-[#6842F5] transition-colors cursor-pointer"
                  >
