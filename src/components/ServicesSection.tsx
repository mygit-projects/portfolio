      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-14 gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <Calculator className="w-4 h-4 text-[#5B3DE0]" />
              <span>{showEstimator ? "Hide Timeline Estimator" : "Scope & Timeline Calculator"}</span>
            </button>

            <button
              onClick={onOpenContact}
              className="px-4 py-2.5 rounded-2xl bg-[#5B3DE0] hover:bg-[#4A2EC4] text-white text-xs font-bold shadow-md shadow-purple-500/20 flex items-center gap-1.5 transition-all cursor-pointer"
            >
            ...
            >
              <div className="bg-white rounded-3xl p-6 sm:p-8 border border-purple-200 shadow-xl shadow-purple-500/5 relative">
                <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 pb-6 mb-6 border-b border-slate-100">
                  <div>
                    <div className="flex items-center gap-2 text-xs font-extrabold text-[#5B3DE0] uppercase tracking-wider mb-1">
                  ...
                  <div className="flex items-center gap-4 bg-purple-50/80 p-3.5 rounded-2xl border border-purple-100">
                    <div>
                      <div className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">Estimated Turnaround</div>
                      <div className="text-2xl font-black text-[#5B3DE0]">
                        {calculatedDays > 0 ? `${calculatedDays}–${calculatedDays + 3} Working Days` : "Select modules"}
                      </div>
                    </div>
                    <button
                      onClick={onOpenContact}
                      className="px-4 py-2.5 rounded-xl bg-[#5B3DE0] hover:bg-[#4A2EC4] text-white text-xs font-bold shadow-sm flex items-center gap-1.5 cursor-pointer"
                    >
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 rounded-2xl border transition-all cursor-pointer select-none flex items-center justify-between ${
                          isSelected
                            ? "bg-purple-50/60 border-[#5B3DE0] ring-2 ring-purple-100 shadow-xs"
                      ...
                      >
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-2 text-[11px] text-slate-500 font-medium">
                            ...
                            <span className="flex items-center gap-0.5 text-[#5B3DE0] font-semibold">
                        ...
                        <div
                          className={`w-6 h-6 rounded-lg flex items-center justify-center border transition-colors ${
                            isSelected ? "bg-[#5B3DE0] border-[#5B3DE0] text-white" : "bg-white border-slate-300 text-transparent"
                          }`}
                        >
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <div className="w-14 h-14 rounded-2xl bg-purple-50 group-hover:bg-[#5B3DE0] group-hover:text-white border border-purple-100 flex items-center justify-center transition-all duration-300 shadow-xs">
                      <Icon className="w-6 h-6 text-[#5B3DE0] group-hover:text-white" />
                    </div>
                    <span className="text-xs font-black text-slate-500 group-hover:text-[#5B3DE0] transition-colors">
                      {service.number}
                    </span>
                  </div>

                  <h3 className="text-lg font-black text-slate-900 mb-2 leading-snug group-hover:text-[#5B3DE0] transition-colors">
                    {service.title}
                  </h3>
                  ...
                  <div className="space-y-2 mb-6 pt-4 border-t border-slate-100">
                    {service.features.map((feature) => (
                      <div key={feature} className="flex items-start gap-2 text-[11px] text-slate-700 font-medium">
                        <CheckCircle2 className="w-3.5 h-3.5 text-[#5B3DE0] shrink-0 mt-0.5" />
                        <span>{feature}</span>
                      </div>
                    ))}
                  </div>
                ...
                <button
                  onClick={onOpenContact}
                  className="w-full py-2.5 rounded-xl bg-slate-50 group-hover:bg-purple-50 text-[#5B3DE0] font-bold text-xs flex items-center justify-center gap-1.5 border border-slate-100 group-hover:border-purple-200 transition-all cursor-pointer"
                >
